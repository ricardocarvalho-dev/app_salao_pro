...
class AgendaPage extends ConsumerStatefulWidget {
  ...
  final DateTime? dataSelecionada; // 🔹 Tornado opcional para flexibilidade

  const AgendaPage({
    super.key,
    required this.salaoId,
    this.dataSelecionada, // 🔹 Pode vir nulo
  });
...
class _AgendaPageState extends ConsumerState<AgendaPage> {
  ...
  void initState() {
    ...
    // 🎯 Define a data inicial: Se veio da notificação usa ela, senão usa Hoje.
    final dataInicial = widget.dataSelecionada ?? DateTime.now();
    
    _selectedDay = dataInicial;
    _focusedDay = dataInicial;
  ...
  @override
  void dispose() {
    // Limpa filtros ao sair para não afetar outras telas
  ...
  */
    ...
    return Column(
      children: [
        SizedBox(
          ...
          child: TableCalendar(
            ...
            daysOfWeekStyle: const DaysOfWeekStyle(
              weekdayStyle: TextStyle(fontSize: 13, color: Colors.black87, height: 1.1),
              weekendStyle: TextStyle(fontSize: 13, color: Colors.redAccent, height: 1.1),
            ),
        ...
        DropdownButtonFormField<String?>(
          ...
          onChanged: (v) {
            ref.read(agendaFiltroProvider.notifier).state =
                AgendaFiltroState(profissionalId: v, servicoId: null);
            carregarAgendamentos();
          },
        ),
        const SizedBox(height: 16),
        DropdownButtonFormField<String?>(
          ...
          onChanged: (v) {
            ref.read(agendaFiltroProvider.notifier).state =
                AgendaFiltroState(profissionalId: null, servicoId: v);
            carregarAgendamentos();
          },
        ),
        const SizedBox(height: 16),
        SizedBox(
          ...
          onPressed: () async {
            ...
            if (dataSelecionada.isBefore(dataHoje)) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text("Não é possível criar agendamentos em datas passadas.")),
              );
              return;
            }
  ...
  Widget _buildLista() {
    if (isLoading) return const AgendaSkeleton();
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        final hora = '${ag.hora.hour.toString().padLeft(2, '0')}:${ag.hora.minute.toString().padLeft(2, '0')}';

        return ListTile(
          title: Text('$hora — ${ag.clienteNome ?? ''}'),
          subtitle: Text(
            '${ag.servicoNome ?? ''} • ${ag.profissionalNome ?? 'Por serviço'}',
          ),
          trailing: IconButton(
